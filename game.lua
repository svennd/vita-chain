...
-- loads
-- local img_sfx = Graphics.loadImage("app0:/assets/sfx.png")
...
local dark_green   = Color.new(0, 128, 0)
local silver   = Color.new(60, 60, 60) -- grey
local deep   = Color.new(0, 0, 120)
local nuke   = Color.new(200, 255, 0)
...
local ATOM = {
         -- {NAME = "SCORE_TEST", SIZE = 20, COLOR = black, EXPAND = 1, FX = SFX>RED_TO_YELLOW, SCORE = 60000},
         {NAME = "HYDROGEN", SIZE = 7, COLOR = yellow, EXPAND = 5, FX = SFX.RED_TO_YELLOW, SCORE = 10}, 
         {NAME = "HELIUM", SIZE = 6.5, COLOR = red, EXPAND = 4, FX = SFX.YELLOW_TO_RED, SCORE = 15},  
         {NAME = "LITHIUM", SIZE = 6, COLOR = green, EXPAND = 3.7, FX = SFX.BLUE_TO_GREEN, SCORE = 25}, 
         {NAME = "BERYLLIUM", SIZE = 5.5, COLOR = blue, EXPAND = 3, FX = SFX.GREEN_TO_BLUE, SCORE = 35},  
         {NAME = "BORON", SIZE = 5, COLOR = purple, EXPAND = 2.7, FX = SFX.ORANGE_TO_PURPLE, SCORE = 50},  
         {NAME = "CARBON", SIZE = 4.5, COLOR = orange, EXPAND = 2.1, FX = SFX.PURPLE_TO_ORANGE, SCORE = 70},
...
function move_atoms(delta)
   ...
   while i < count_atoms do
      ...
      if c_atom.state == STATE.INIT then
         ...
         -- determ new location
         local new_x = current_x + dir_x*delta -- \/
...
function game_start()
   ...
   local timestep = 1000/30 -- 60 fps target -- 30 fixes speedups and actually runs at ~144
   local delta = 0
   ...
   while true do
      ...
      while delta >= timestep do
         ...
         -- escape spiral of death, should not occur -- fixed
...
function user_input()
   ...
   local now = Timer.getTime(game_play)
   local dt = now - game_last_input 
...
function load_levels()
   ...
   -- level 16-20
   add_level(30, 45, 6, "yeah\njust do it\nGET30 out of 45") -- 66%
   add_level(38, 50, 5, "let the lagg begin\nGET38 out of 50") -- 76%
   add_level(38, 50, 6, "but can you do it\nagain?\nGET38 out of 50") -- 76%
   add_level(43, 53, 6, "good job\nGET43 out of 53") -- 81%
   add_level(48, 55, 6, "nice\nGET48 out of 55") -- 87%

   -- level 21-25
   add_level(20, 50, 7, "dark green is tiny\nGET20 out of 50") -- 40%
   add_level(37, 59, 8, "dark blue?\nGET37 out of 59") -- 55%?
   add_level(50, 60, 9, "black???\nGET50 out of 60") -- 92%?
   add_level(3, 10, 9, "ready for the final level?\nGET3 out of 10") -- 30%
   add_level(61, 60, 10, "megumin would be proud\nthis is impossible\nthose super small ones are fun\nGET61 out of 60") -- 101%?
end
